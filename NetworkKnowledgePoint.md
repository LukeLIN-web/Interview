...
1. 校验， 检查源和目的地 ip 地址 ， 还检查 数据。
2. 序号
3. 确认， 默认用累计确认，细节看下面的连续ARQ协议。
...
**优点：** 信道利用率高，容易实现，即使确认丢失，也不必重传。  **缺点：** 不能向发送方反映出接收方已经正确收到的所有分组的信息。 比如：发送方发送了 5条 消息，中间第三条丢失（3号），这时接收方只能对前两个发送确认。发送方无法知道后三个分组的下落，而只好把后三个全部重传一次。
...
- **拥塞避免：**  > 慢开始门限后，用拥塞避免， 每经过一个往返时间RTT就把发送放的cwnd加1.
...
- 可以通过设置SOCKET选项来重用处于TIMEWAIT的sock地址，这个参数不是“消除” TIME_WAIT的，而是说当资源不够时，可以重用TIME_WAIT的连接
...
不用keepalive的三个理由：（1）在短暂的故障期间，它们可能引起一个良好连接（good connection）被释放（dropped），（2）它们消费了不必要的宽带，（3）在以数据包计费的互联网上它们（额外）花费金钱。然而，在许多的实现中提供了存活定时器。**Keepalive适用于清除死亡时间比较长的连接。**  
